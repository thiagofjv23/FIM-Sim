...
function triggerFullSeason() {
    const btn = document.getElementById('btnSeason');
    ...
    function runNext() {
        try {
            ...
            if (typeof logEvent === 'function') logEvent(`⚠️ Erro: ${err.message}`, 'warn');
            if (btn) btn.disabled = false;
            if (btnSim) btnSim.disabled = false;
        }
    }
...
function shouldRenew(rider, teamObj) {
    // Score de performance: posição relativa na última temporada (0-100, 100 = venceu)
    const perfScore = rider.lastSeasonTotal
        ? Math.max(0, 100 - ((rider.lastSeasonPos - 1) / rider.lastSeasonTotal) * 100)
        : 50; // sem dados → assume médio

    if (teamObj.aiPersonality === 'caca_talentos') {
        // 80% atributo (potencial), 20% performance
        const score = rider.potential * 0.80 + perfScore * 0.20;
        return score >= 75;
    }
    if (teamObj.aiPersonality === 'conservadora') {
        // 60% atributo (velocidade + consistência), 40% performance
        const atributo = (rider.speed + rider.consistency) / 2;
        const score = atributo * 0.60 + perfScore * 0.40;
        return score >= 65;
    }
    if (teamObj.aiPersonality === 'resultados_imediatos') {
        // 30% atributo (velocidade), 70% performance
        const score = rider.speed * 0.30 + perfScore * 0.70;
        return score >= 60;
    }
    // padrão
    const score = rider.speed * 0.50 + perfScore * 0.50;
    return score >= 62;
}

function calcHireScore(rider, teamObj) {
    // Score composto para ordenar candidatos no mercado de contratações
    const perfScore = rider.lastSeasonTotal
        ? Math.max(0, 100 - ((rider.lastSeasonPos - 1) / rider.lastSeasonTotal) * 100)
        : 30; // novatos sem histórico penalizados lightly

    if (teamObj.aiPersonality === 'caca_talentos')
        return rider.potential * 0.70 + rider.speed * 0.20 + perfScore * 0.10;
    if (teamObj.aiPersonality === 'conservadora')
        return ((rider.speed + rider.consistency) / 2) * 0.50 + rider.potential * 0.30 + perfScore * 0.20;
    if (teamObj.aiPersonality === 'resultados_imediatos')
        return rider.speed * 0.30 + rider.potential * 0.20 + perfScore * 0.50;
...
function runYearEndTransfers() {
    ...
    // Passo 0-A: rescisão antecipada por desempenho (equipes exigentes)
    for (const cat in ecosystem) {
        ...
        ecosystem[cat] = ecosystem[cat].filter(r => {
            if (!r.contractEndYear || r.contractEndYear >= currentYear) {
                const teamObj = config.teams.find(t => t.id === r.teamId);
                if (!teamObj) return true;
                // Só equipes 'resultados_imediatos' com metas ambiciosas (≤ 5) podem rescindir
                if (teamObj.aiPersonality !== 'resultados_imediatos') return true;
                if (!teamObj.targetPosition || teamObj.targetPosition > 5) return true;
                // Rescinde se piloto ficou mais de 8 posições abaixo do esperado
                if (r.lastSeasonPos && r.lastSeasonCat === cat &&
                    ...
                    if (typeof logEvent === 'function')
                        logEvent(`📤 ${r.flag} ${r.name} rescindido por ${teamObj.name} (${motivo})`, 'warn');
                    return false;
                }
            }
            return true;
...
function _triggerSimulationCore() {
    ...
    if (currentRound >= totalRoundsPerSeason) {
        ...
        // Guardar performance da temporada antes do reset (usada nas transferências)
        for (const cat in ecosystem) {
            const sorted = [...ecosystem[cat]].sort((a, b) => b.points - a.points);
            sorted.forEach((r, idx) => {
                r.lastSeasonPoints  = r.points;
                r.lastSeasonPos     = idx + 1;
                r.lastSeasonCat     = cat;
                r.lastSeasonTotal   = sorted.length;
            });
        }
